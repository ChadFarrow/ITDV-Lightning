...
const NowPlayingScreen: React.FC<NowPlayingScreenProps> = ({ isOpen, onClose }) => {
  ...
  const handleViewAlbum = () => {
    if (currentAlbum) {
      // If we're in "Shuffle All" mode, check for special cases
      if (currentAlbum === 'Shuffle All') {
        // For promo tracks with website field, redirect to external site
        if (currentTrack?.website) {
          window.open(currentTrack.website, '_blank');
          onClose();
          return;
        }
        
        // For tracks with actual album, navigate to that album
        ...
        // If no album and no website, just close (avoid broken links)
        onClose();
        return;
      }
    ...
    >
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-4 sm:px-8 pb-4 sm:pb-8" style={{ paddingBottom: `max(2rem, calc(2rem + env(safe-area-inset-bottom)))` }}>
        ...
        <div className="w-full max-w-md text-center mb-8">
          ...
          {/* Show website link for promo tracks */}
          {currentTrack.website && (
            <div className="mt-3">
              <a 
                href={currentTrack.website}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 px-4 py-2 bg-white/10 hover:bg-white/20 text-white text-sm rounded-full transition-colors border border-white/20 hover:border-white/40"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 12a9 9 0 01-9 9m9-9a9 9 0 00-9-9m9 9H3m9 9a9 9 0 01-9-9m9 9c1.657 0 3-4.03 3-9s-1.343-9-3-9m0 18c-1.657 0-3-4.03-3-9s1.343-9 3-9m-9 9a9 9 0 019-9" />
                </svg>
                <span>Visit Doerfelverse</span>
